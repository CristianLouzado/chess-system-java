...
public class ChessMatch {
	...
	public ChessPiece perfomeChessmove(ChessPosition sourcePosition, ChessPosition targetPosition) {
		...
		/*
		 * if(testCheck(currentPlayer)) { undoMove(source, target, capturedPiece); throw
		 * new ChessException("You can't put yourself in check."); } check =
		 * (testCheck(opponent(currentPlayer)))? true :false;
		 */
		nextTurn();
	 ...
	 * king(color).getChessPosition().toPosition(); List<Piece> opponentPieces =
	 * piecesOnTheBoard.stream().filter(x -> ((ChessPiece)x).getColor() ==
	 * opponent(color)).collect(Collectors.toList()); for (Piece p : opponentPieces)
	 * { boolean[][] mat = p.possibleMoves(); if
	 * (mat[kingPosition.getRow()][kingPosition.getColumn()]) { return true; } }
	 * return false; }
	 */
